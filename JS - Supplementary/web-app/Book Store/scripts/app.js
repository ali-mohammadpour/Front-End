...
document.querySelector("#add-product-form").addEventListener("submit", (event) => {   // baraye zakhire dade az submit estefade mikonim.
...
document.querySelector("#search-products").addEventListener("input", (event) => {
    filters.searchItem = event.target.value   // daryafte inpute search
    renderProducts(products, filters)
})

document.querySelector("#available-products").addEventListener("change", (event) => {
    filters.availableProducts = event.target.checked
    renderProducts(products, filters)
})

window.addEventListener("storage", (e) => {   // baraye safe index.html
    if(e.key === "products"){
